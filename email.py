...
class Email(object):
    ...
    def read_password(self):
        ...
        buf = ''
        while not buf and not buf.startswith('[sendgrid]') :
            buf = f.readline()
        if buf:
            name = f.readline()[:-1]
            pwd = f.readline()[:-1]
        else:
            print 'ERROR: Email component password is not set correctly in password file'
        f.close()
        return name, pwd

    def send(self, to_list, subject, text, html='', attachments=[], bcc_list=[]):

        if len(to_list) + len(bcc_list) == 0:
            print 'No email receiver specified.'
            return
